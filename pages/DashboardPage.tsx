...
import { Calendar, MapPin, User, Utensils, IndianRupee, Trash2, CheckCircle, XCircle, Database, RefreshCw, BarChart3, CloudDownload } from 'lucide-react';
...
const DashboardPage: React.FC = () => {
  ...
  const handleCancel = async (id: string) => {
    if (window.confirm("Are you sure you want to cancel this booking? This action will immediately release your seat.")) {
      setCancellingId(id);
      try {
                    ...
                    <div className="flex items-center gap-4 md:gap-10">
                      ...
                      <div className="flex-1 flex flex-col md:flex-row md:items-center gap-2 md:gap-8">
                        ...
                        <div className="md:text-right">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">Destination</p>
                          <p className="text-xl font-black text-slate-900">{getStationName(booking.toStationId)}</p>
                        </div>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        <p className="text-[9px] font-black text-slate-400 uppercase flex items-center gap-1"><CheckCircle size={10} /> Seat ID</p>
                        <p className="text-xs font-bold text-slate-700">{booking.seatId}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[9px] font-black text-slate-400 uppercase flex items-center gap-1"><Utensils size={10} /> Add-on</p>
                        <p className="text-xs font-bold text-slate-700 truncate">{getMealName(booking.mealId)}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[9px] font-black text-slate-400 uppercase flex items-center gap-1"><IndianRupee size={10} /> Paid</p>
